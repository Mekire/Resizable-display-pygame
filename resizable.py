...
class Control(object):
    ...
    def __init__(self):
        ...
        self.display = pg.Surface(self.screen_rect.size).convert()
    ...
    def render(self):
        ...
        if self.screen_rect.size != SCREEN_START_SIZE:
            fit_rect = self.display_rect.fit(self.screen_rect)
            fit_rect.center = self.screen_rect.center
            scaled = pg.transform.smoothscale(self.display, fit_rect.size)
            self.screen.blit(scaled, fit_rect)
        else:
